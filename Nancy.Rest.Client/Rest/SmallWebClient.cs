...
namespace Nancy.Rest.Client.Rest
{
    public class SmallWebClient
    {
        ...
        public static async Task<object> RestRequest(Request req, IWebProxy proxy=null)
        {
            ...
            if (req.ReturnType.IsAssignableFrom(typeof(Stream)) || req.ReturnType == typeof(object))
            {
                returnasstream = true;
                accept = "*/*";
            }
            ...
            content.Dispose();
            return JsonConvert.DeserializeObject(responseString, req.ReturnType, req.SerializerSettings);
        }
    }
}
