...
namespace JMMClient.Downloads
{
	public class UTorrentHelper
	{
		...
		public void Init()
		{
			...
			UTorrentHelperVM.Instance.ConnectionStatus = "Populating security token...";
			PopulateToken();

			// timer for automatic updates
			torrentsTimer = new System.Timers.Timer();
			torrentsTimer.AutoReset = false;
			torrentsTimer.Interval = UserSettingsVM.Instance.UTorrentRefreshInterval * 1000; // 5 seconds
		...
		void torrentsTimer_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
		{
			...
			try
			{
				if (!UserSettingsVM.Instance.UTorrentAutoRefresh) return;
				...
				if (success)
				{
					...
					torrentsTimer.Interval = UserSettingsVM.Instance.UTorrentRefreshInterval * 1000;
				}
				else
