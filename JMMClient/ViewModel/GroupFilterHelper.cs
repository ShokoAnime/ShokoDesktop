	...
	public class GroupFilterHelper
	{
		...
		public static List<GroupFilterVM> GetGroupFiltersFromDatabase()
		{
			...
			try
			{
				...
				if (gfs.Count == 0)
				{
					// if there are no group filters (new user), lets creates some default ones
					...
					// Missing Episodes
					gf = new GroupFilterVM();
					gf.FilterConditions = new ObservableCollection<GroupFilterConditionVM>();

					gfc = new GroupFilterConditionVM();
					gfc.ConditionType = (int)GroupFilterConditionType.MissingEpisodes;
					gfc.ConditionOperator = (int)GroupFilterOperator.Include;
					gf.FilterConditions.Add(gfc);

					gf.AllowEditing = true;
					gf.FilterName = "Missing Episodes";
					gf.ApplyToSeries = 0;
					gf.BaseCondition = 1;
					gfilters.Add(gf);
					...
					gf.AllowEditing = true;
					gf.FilterName = "Newly Added Series";
					gf.ApplyToSeries = 0;
					gf.BaseCondition = 1;
					gfilters.Add(gf);
					...
					gf.AllowEditing = true;
					gf.FilterName = "Newly Airing Series";
					gf.ApplyToSeries = 0;
					gf.BaseCondition = 1;
					gfilters.Add(gf);
					...
					gfc = new GroupFilterConditionVM();
					gfc.ConditionType = (int)GroupFilterConditionType.UserVoted;
					gfc.ConditionOperator = (int)GroupFilterOperator.Exclude;
					gf.FilterConditions.Add(gfc);

					gf.AllowEditing = true;
					gf.FilterName = "Votes Needed";
					gf.ApplyToSeries = 0;
					gf.BaseCondition = 1;
					gfilters.Add(gf);
					...
					gf.AllowEditing = true;
					gf.FilterName = "Recently Watched";
					gf.ApplyToSeries = 0;
					gf.BaseCondition = 1;
					gfilters.Add(gf);
		...
		public static List<GroupFilterVM> AllGroupFilters
		{
			get
			{
				...
				// Favorites
				GroupFilterVM gf = new GroupFilterVM();
				gf.FilterConditions = new List<GroupFilterCondition>();

				GroupFilterCondition gfc = new GroupFilterCondition();
				gfc.ConditionType = GroupFilterConditionType.Favourite;
				gfc.ConditionOperator = GroupFilterOperator.Include;
				gf.FilterConditions.Add(gfc);

				gf.AllowEditing = false;
				gf.FilterName = JMMClient.Properties.Resources.GroupFilter_Favorites;
				gfilters.Add(gf);

				#endregion

				#region NOT Favorites
				...
				gfc.ConditionType = GroupFilterConditionType.Favourite;
				gfc.ConditionOperator = GroupFilterOperator.Exclude;
				gf.FilterConditions.Add(gfc);

				gf.AllowEditing = false;
				gf.FilterName = "Not Faves";
				gfilters.Add(gf);

				#endregion

				#region Missing Episodes
				...
				gfc = new GroupFilterCondition();
				gfc.ConditionType = GroupFilterConditionType.MissingEpisodes;
				gfc.ConditionOperator = GroupFilterOperator.Include;
				gf.FilterConditions.Add(gfc);
				...
				#region Favorites with Missing Episodes

				// Favorites with Missing Episodes
				gf = new GroupFilterVM();
				gf.FilterConditions = new List<GroupFilterCondition>();
				...
				gf.AllowEditing = false;
				gf.FilterName = "Favorites with Missing Episodes";
				gfilters.Add(gf);

				#endregion

				#region Favorites with Unwatched Episodes

				// Favorites with Unwatched Episodes
				gf = new GroupFilterVM();
				gf.FilterConditions = new List<GroupFilterCondition>();
				...
				gfc = new GroupFilterCondition();
				gfc.ConditionType = GroupFilterConditionType.HasUnwatchedEpisodes;
				gfc.ConditionOperator = GroupFilterOperator.Include;
				gf.FilterConditions.Add(gfc);

				gf.AllowEditing = false;
				gf.FilterName = "Favorites with Unwatched Episodes";
				gfilters.Add(gf);

				#endregion

				#region Unwatched Complete Series
				...
				gf.AllowEditing = false;
				gf.FilterName = "Unwatched Complete Series";
				//gfilters.Add(gf);

				#endregion
				*/
				// load the user created filters
