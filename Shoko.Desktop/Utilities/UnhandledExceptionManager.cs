    ...
    public sealed class UnhandledExceptionManager
    {
        ...
        private static string StackFrameToString(StackFrame sf)
        {
            ...
            if (sf.GetFileName() == null || sf.GetFileName().Length == 0)
            {
                ...
                _with1.Append(": N ");
                _with1.Append($"{sf.GetNativeOffset():#00000}");
            }
            else
            {
                _with1.Append(System.IO.Path.GetFileName(sf.GetFileName()));
                _with1.Append(": line ");
                _with1.Append($"{sf.GetFileLineNumber():#0000}");
                _with1.Append(", col ");
                _with1.Append($"{sf.GetFileColumnNumber():#00}");
                //-- if IL is available, append IL location info
                if (sf.GetILOffset() != StackFrame.OFFSET_UNKNOWN)
                {
                    _with1.Append(", IL ");
                    _with1.Append($"{sf.GetILOffset():#0000}");
                }
            }
            _with1.Append(Environment.NewLine);
            return sb.ToString();
        ...
        private static void ExceptionToEventLog()
        {
            try
            {
                EventLog.WriteEntry(AppDomain.CurrentDomain.FriendlyName, Environment.NewLine + _strException, EventLogEntryType.Error);
